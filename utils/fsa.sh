...
function sanity_check {
    ...
    if [ -d host_vars ]; then
        ...
        varsdir="host_vars"
        ansibledir="$(pwd)"
        versionstring="FSA_VERSION_STRING-dev"
        ...
        varsdir="config"
        ansibledir="$(pwd)"
        versionstring="FSA_VERSION_STRING"
    else
    ...
    roles=$(find $ansibledir/roles -maxdepth 1 -type d | sort)
    k8s=""
    vms=""
    internal=""
    ...
    for role in $roles; do
        ...
        echo $works_against | grep -q 'FSA-Internal' && internal+=" $role_name"
        echo $works_against | grep -qe 'OpenBSD' -e 'Alpine' && vms+=" $role_name"
    done;

    # piping output to xargs without any parameters will trim the string
    vms=$(echo $vms | xargs)
    k8s=$(echo $k8s | xargs)
    internal=$(echo $internal | xargs)
...
function map_tags {
    ...
    # concatenate vms and k8s roles
    availableroles=$("$vms $k8s" | tr ' ' ',')
...
function build_playbook {
    ...
    # the list of k8s roles is very small so it is hardcoded here for now
    cat << EOF > "$playbookfile"
# fsa $versionstring
