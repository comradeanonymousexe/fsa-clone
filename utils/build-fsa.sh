...
# it's a git submodule here so we rm -rf it to have it back as a normal folder
rm -rf ./config/
mkdir ./config/
...
if find ./ -type f -name '*.md' -not -path "./terraform/*" | xargs grep FSA_VERSION_STRING; then
