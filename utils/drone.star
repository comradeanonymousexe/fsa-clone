...
def push_fsa(push, keyname, depends):
    ...
    prdcommands = basecommands + [
        # publish to prod
        "export reponame='fsa'",
        "export remotename='ssh://git@github.com/yviel-de:fsa'",
        "bash -x utils/push-fsa.sh --fsa"
    ]
    steps = [
        step_wrapper("push-stage", stgcommands, environment=environment, image="registry.opviel.de:80/fsa-build"),
    ]
    if push == "prod":
        steps.append(step_wrapper("push-prod", prdcommands, environment=environment, image="registry.opviel.de:80/fsa-build"))
    return pipeline_wrapper("push-fsa", steps, depends)
